...
class UserModel {
  ...
  const UserModel({
    ...
    required this.role, this.avatar = '', this.rating = 4.8,
    this.projectsCount = 0, this.skills = const [],
  });
  String get initials {
    final p = name.split(' ');
...
class TaskModel {
  ...
  const TaskModel({
    ...
    required this.assigneeInitials, required this.status,
...
class ProjectModel {
  ...
  const ProjectModel({
    required this.id, required this.name, required this.company,
    this.description = '', required this.status, required this.delayRisk,
    required this.progress, this.tasks = const [], this.members = const [],
  });
}
...
class ChatMessage {
  ...
  const ChatMessage({
    ...
    required this.senderInitials, required this.message,
    required this.time, this.isMe = false,
  });
}
...
class ChatRoom {
  ...
  const ChatRoom({
    ...
    this.unread = 0, this.isGroup = false,
  });
}
...
class LoginLog {
  ...
  const LoginLog({
    ...
    required this.device, required this.ip,
  });
...
class FileItem {
  ...
  const FileItem({
    required this.id, required this.name, required this.size,
    required this.type, required this.uploadedBy,
    required this.date, this.status = 'Verified',
  });
}
