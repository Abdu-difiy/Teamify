...
class TCard extends StatelessWidget {
  ...
  final Color? color;
  const TCard({super.key, required this.child, this.padding, this.margin, this.onTap, this.color});
...
class TButton extends StatelessWidget {
  ...
  final IconData? icon;
  final bool outline;
  const TButton({super.key, required this.label, this.onTap, this.icon, this.outline = false});
    ...
    return SizedBox(
      ...
      child: outline
          ? OutlinedButton(
              ...
              child: Text(label, style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600, fontSize: 16)),
            )
          : ElevatedButton(
              onPressed: onTap,
...
class TAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CircleAvatar(
      ...
      child: Text(initials, style: TextStyle(color: Colors.white, fontSize: radius * 0.65, fontWeight: FontWeight.bold)),
    );
  }
}
...
class TChip extends StatelessWidget {
  ...
  const TChip({super.key, required this.label, this.bg, this.textColor, this.fontSize = 11});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label, style: TextStyle(fontSize: fontSize, color: textColor ?? AppColors.primary, fontWeight: FontWeight.w600)),
    );
  }
}
              ...
              return Expanded(
                child: GestureDetector(
                  ...
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(sel ? items[i]['activeIcon'] as IconData : items[i]['icon'] as IconData,
                          color: sel ? AppColors.primary : AppColors.textSecondary, size: 24),
                      const SizedBox(height: 2),
                      Text(items[i]['label'] as String,
                          style: TextStyle(fontSize: 11, color: sel ? AppColors.primary : AppColors.textSecondary,
...
class StatBox extends StatelessWidget {
  ...
  final IconData icon;
  final Color? iconBg;
  const StatBox({super.key, required this.value, required this.label, required this.icon, this.iconBg});
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: (iconBg ?? AppColors.primary).withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
...
class AIBanner extends StatelessWidget {
  ...
  final VoidCallback? onTap;
  const AIBanner({super.key, required this.title, required this.subtitle, this.badge = '', this.onTap});
      ...
      child: Row(
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                if (badge.isNotEmpty) ...[
                  ...
                  Text(badge, style: const TextStyle(fontSize: 12, color: AppColors.success, fontWeight: FontWeight.w600)),
                ],
              ],
            ),
          ),
          if (onTap != null)
            GestureDetector(
              onTap: onTap,
              child: const Text('Review now →', style: TextStyle(fontSize: 12, color: AppColors.primary, fontWeight: FontWeight.w600)),
            ),
        ],
      ),
    );
...
void handleFreelancerNav(BuildContext ctx, int i) {
  switch (i) {
    case 0: Navigator.pushReplacementNamed(ctx, R.freelancerHome); break;
    case 1: Navigator.pushNamed(ctx, R.search); break;
    case 2: Navigator.pushNamed(ctx, R.aiHub); break;
    case 3: Navigator.pushNamed(ctx, R.chatList); break;
    case 4: Navigator.pushNamed(ctx, R.freelancerProfile); break;
  }
}
